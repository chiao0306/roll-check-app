...
def audit_with_gemini(extracted_data_list, api_key):
    ...
    **請建立每一支滾輪編號的完整履歷，並執行以下比對：**
    ...
    #### A. 【本體 (Body)】未再生/車修 - 【強制最大值原則】：
    - **規格解析 (Spec Parsing)**：
      - 請掃描該項目列出的所有尺寸數字 (忽略「每次車修」、「驅動/非驅動」、「至 Ymm」等所有文字敘述)。
      - **關鍵規則**：若出現多個目標尺寸 (例如 294mm 與 285mm)，請直接比較數值大小，**一律取數值最大者** 作為唯一的通用比對標準 (Max_Spec)。
      - 範例：規範含 294mm 與 285mm -> Max_Spec = 294。
    - **特別聲明**：本體未再生 **允許** 出現小數點 (代表已完工)。
    - **邏輯分流**：
      1. **整數** (未完工)：實測值 **<=** Max_Spec。
      2. **小數** (已完工)：實測值 **>=** Max_Spec 且 格式為 `#.##`。
    ...
    - **邏輯防呆**：
      - **注意**：銲補是「加法」，數值越大越好。
      - **判定範例**：若規格 233，實測 236。因為 236 > 233，所以是 **PASS (合格)**。
      - 嚴禁將「未再生(越小越好)」的邏輯套用在此處。
