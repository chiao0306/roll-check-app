...
def audit_with_gemini(extracted_data_list, api_key):
    ...
    **請建立每一支滾輪編號的完整履歷，並執行以下比對：**
    ...
    - **研磨限制 (Grinding Check)**：
      - 若同一編號同時存在「再生車修」與「研磨」數據，**研磨尺寸 必須小於 再生車修尺寸** (物理上研磨為去除材料)。
      - 若 研磨 >= 再生 -> **FAIL (邏輯異常：研磨後尺寸變大)**。
    - 以 **「最終完成尺寸」** (再生車修或研磨) 為基準 (Base)。
    - **本體 (Body)**：
      - 未再生 (往下跳)：`Base - 未再生` 必須 <= 30mm。
      - 銲補 (往上跳)：`銲補 - Base` 必須 <= 8mm。
    - **軸頸 (Journal)**：
      - 未再生 (往下跳)：`Base - 未再生` 必須 <= 6mm。
      - 銲補 (往上跳)：`銲補 - Base` 必須 <= 8mm。
    - **異常**：若跳動幅度超過上述範圍 -> **FAIL (尺寸異常：數值不連貫)**。

    ### 2. 數量與依賴性檢查 (Quantity & Dependency) - 【單位邏輯更新】：
    - **熱處理**：忽略數量單位，只要有數據即 PASS。
    - **單位換算與目標數量 (Unit Conversion)**：
      - **(1SET=4PCS)**：若項目標註此規則，目標數量 = 標示SET數 × 4。
      - **(SET)**：若僅標註 SET 且無說明，預設 **1 SET = 2 PC**。目標數量 = 標示SET數 × 2。
      - **(PC)**：目標數量 = 標示 PC 數。
      - **通用規則**：凡項目名稱含上述單位者，皆須核對實測總數是否符合目標數量。
    ...
    #### E. 內孔車修 (Inner Hole)：
    - **規格對應 (Size Mapping)**：請比對該編號在 **「軸頸再生」** 的實測尺寸。
      - 若軸頸 $\approx$ 85mm -> 應符合 **50mm** 的內孔規範。
      - 若軸頸 $\approx$ 75mm -> 應符合 **45mm** 的內孔規範。
    - **邏輯**：實測值需在對應規範的公差範圍內。
    - **格式**：必須精確到小數點後兩位。
